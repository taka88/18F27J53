...
//    I2C1_Write1ByteRegister(ST7032_ADRES, 0x00, c);
//    if (c & 0xFC) // 上位6ビットに1がある命令の場合
//        __delay_us(30); // >26.3us
//    else
//        __delay_ms(2); // >1.08ms ClearおよびHomeコマンド
     int  ans ;
...
{
//    I2C1_Write1ByteRegister(ST7032_ADRES, 0x40, c);   //RS=1
//    __delay_us(30); // >26.3us
     int  ans ;
//     printf("4-1\r\n");
     ans = I2C_Start(ST7032_ADRES,RW_0);     // スタートコンディションを発行する
...
////    while (*s) //文字列の終わり(00)まで継続
////        LCD_Putc(*s++); //文字出力しポインタ＋１
     int  ans ;
//     I2C_SSPCON1= 0x28;
...
//LCD_Init_ST7032i(0,50,1,16);
{
     ...
     __delay_ms(50);                                 // 電源ＯＮ後40msまで待ってから初期化
     FuncSet_DT = 0x38 ;
     ...
     ContSet_DT = d ;
     LCD_Command(ContSet_DT) ;        // Contrast set           : 昇圧回路、コントラスト調整データ
     LCD_Command(0x6C) ;     // Follower control       : フォロア回路をON、増幅率の調整を行う
     __delay_ms(200);                                // 電力が安定するまで待つ(200ms)
