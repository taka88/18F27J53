...
void BME280_loop()
{
 ...
 DI = 0.81*temp_act+0.01*hum_act*(0.99*temp_act-14.3)+46.3;
    ...
    sprintf(buf,"%5.1fßC %5.1f%%", temp_act, hum_act);
    printf("%5.1f ßC %5.1f %%\r\n", temp_act, hum_act);
    LCD_SetCursor(0,0);
    LCD_Puts(buf);
...
void BME280readTrim()
{
    ...
    ans =I2C_Start(0x76,0);
    if (ans == 0) {
        I2C_Send(0x88);
    }

    ans =I2C_rStart(0x76,1);
    for(int8_t i=0; i<=22; i++){
      data[i] = I2C_Receive(0);
    }
    data[23] = I2C_Receive(1);
    I2C_Stop();
